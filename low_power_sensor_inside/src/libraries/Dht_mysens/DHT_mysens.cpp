...
#include "DHT_mysens.h"

void DHT_mysens::setup(uint8_t pin, DHT_MODEL_t model)
{
  DHT_mysens::pin = pin;
  DHT_mysens::model = model;
  DHT_mysens::resetTimer(); // Make sure we do read the sensor in the next readSensor()

  if ( model == AUTO_DETECT) {
    DHT_mysens::model = DHT22;
    readSensor();
    if ( error == ERROR_TIMEOUT ) {
      DHT_mysens::model = DHT11;
...
void DHT_mysens::resetTimer()
{
  DHT_mysens::lastReadTime = millis() - 3000;
}

float DHT_mysens::getHumidity()
{
  readSensor();
  return humidity;
}

float DHT_mysens::getTemperature()
{
  readSensor();
  return temperature;
}
...
const char* DHT_mysens::getStatusString()
{
  switch ( error ) {
    case DHT_mysens::ERROR_TIMEOUT:
      return "TIMEOUT";

    case DHT_mysens::ERROR_CHECKSUM:
      return "CHECKSUM";
...
void DHT_mysens::readSensor(bool force)
{
